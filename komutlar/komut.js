...
exports.run = async (client, message, args) => {
	...
	if (args[0] == "ekle") {
		...
		if (dmtakip == "aktif") {
			...
			if (!kanal)
				return message.channel
					.send("Botlist kanal ayarlanmamış!")
					.then(message => setTimeout(() => message.delete(), 5000));
			let yetkilirol = db.fetch(`botlistyetkili_${message.guild.id}`);
			if (!yetkilirol)
				return message.channel
					.send("Botlist yetkili rolü ayarlanmamış!")
					.then(message => setTimeout(() => message.delete(), 5000));
			let devrol = db.fetch(`botlistdevrol_${message.guild.id}`);
			if (!devrol)
				return message.channel
					.send("Botlist devepoler rolü ayarlanmamış!")
					.then(message => setTimeout(() => message.delete(), 5000));
			let başvurulog = db.fetch(`botlistbaşvurulog_${message.guild.id}`);
			if (!başvurulog)
				return message.channel
					.send("Botlist başvuru log kanalı ayarlanmamış!")
					.then(message => setTimeout(() => message.delete(), 5000));
			let log = db.fetch(`botlistlog_${message.guild.id}`);
			if (!log)
				return message.channel
					.send("Botlist log ayarlanmamış!")
					.then(message => setTimeout(() => message.delete(), 5000));
			const kanalhata = new Discord.MessageEmbed()
				.setTitle("Sennpai bot hata")
				.setDescription(
					":name_badge:  Burası bot ekleme kanalı değil!  :name_badge:"
				);
			if (message.channel.id !== kanal)
				return message.channel
					.send(kanalhata)
					.then(message => setTimeout(() => message.delete(), 3500));
			let id = args[1];
			if (!id)
				return message.channel
					.send("Lütfen idi giriniz!")
					.then(message => setTimeout(() => message.delete(), 3500));
			let prefix = args[2];
			if (!prefix)
				return message.channel
					.send("lütfen prefixi giriniz!")
					.then(message => setTimeout(() => message.delete(), 3500));
			let dbldurum = args[3];
			if (!dbldurum)
				return message.channel
					.send("Lütfen dbl durumunu giriniz!")
					.then(message => setTimeout(() => message.delete(), 3500));
			...
			if (sıravar == undefined) {
				var sıra = "sıra yok";
			}
			client.channels.cache
				.get(log)
				.send(`<@${message.author.id}>, <@&${yetkilirol}>`);
			...
			const başvurulogembed = new Discord.MessageEmbed()
				.setTitle(`Bot başvurusu`)
				.setDescription(
					`__**BAŞVURU YAPANIN BİLGİLERİ**__ \n Başvuru yapan: <@${message.author.id}> \n İdi: ${message.author.id} \n \n __**BAŞVURU YAPILAN BOT BİLGİLERİ**__ \n Bot: <@${id}> \n Prefixi: ${prefix} \n Dbl onay durumu: ${dbldurum}\n 8perm: [TIKLA](https://discord.com/api/oauth2/authorize?client_id=${id}&permissions=8&scope=bot) \n 0perm: [TIKLA](https://discord.com/api/oauth2/authorize?client_id=${id}&permissions=0&scope=bot)`
				);
			...
			const embed = new Discord.MessageEmbed()
				...
				.setDescription(
					message.guild.name +
						` adlı sunucudaki bot başvurunuz alınmıştır.Başvurunuzla ilgili bir gelişme olduğu zaman size buradan bildiride bulunacağım \n \n Eğer anlık bilgi almak istersen s.bot bilgi ${id}`
				);
			message.author.send(embed);
			return message.channel
				.send("Başvurunuz alınmıştır!")
				.then(message => setTimeout(() => message.delete(), 3500));
		}
		message.delete();
		let kanal = db.fetch(`botlistkanal_${message.guild.id}`);
...
`);
	...
	if (args[0] == "onayla") {
		...
		if (dmtakip == "aktif") {
			...
			if (
				!message.member.roles.cache.has(
					db.fetch(`botlistyetkili_${message.guild.id}`)
				)
			) {
				return message.channel.send(
					"Bu Komutu sadece botlist yetkilisi kullanabilir!"
				);
			}
			let pythonic = args[1];
			if (!pythonic)
				return message.channel.send(
					"Lütfen onaylanacak botun idini vbelirtin!"
				);
			let sahip = db.fetch(`botbaşvuruid_${message.guild.id}.${pythonic}`);
			if (!sahip)
				return message.channel.send("Böyle bir bot başvurusu yapılmamış!");
			...
			const logembed = new Discord.MessageEmbed()
				...
				.setDescription(
					`<@${message.author.id}> adlı yetkili <@${sahip}> adlı kişinin <@${pythonic}> adlı botunu onayladı!`
				);
			client.channels.cache.get(log).send(logembed);
			...
			const onaylandı = new Discord.MessageEmbed()
				...
				.setDescription(
					`<@${message.author.id}> adlı kişi, <@${sahip}> adlı kişinin <@${pythonic}> adlı botunu onayladı!`
				);
			const embedd = new Discord.MessageEmbed()
				.setTitle("Botunuz onaylandı!")
				.setDescription(
					`<@${message.author.id}> adlı yetkili <@${pythonic}> adlı botunuzu onayladı!`
				);
		...
		let pythonic = args[1];
		if (!pythonic)
			return message.channel.send("Lütfen onaylanacak botun idini vbelirtin!");
		let sahip = db.fetch(`botbaşvuruid_${message.guild.id}.${pythonic}`);
		if (!sahip)
			return message.channel.send("Böyle bir bot başvurusu yapılmamış!");
		let sıra = db.fetch(`botlistsıra_${message.guild.id}`);
	...
	if (args[0] == "reddet") {
		...
		if (dmtakip == "aktif") {
			...
			if (!pythonic)
				return message.channel.send(
					"Lütfen redddedilecek botun idini belirtin!"
				);
			let sahip = db.fetch(`botbaşvuruid_${message.guild.id}.${pythonic}`);
			if (!sahip)
				return message.channel.send("Böyle bir bot başvurusu yapılmamış!");
			...
			const logembed = new Discord.MessageEmbed()
				...
				.setDescription(
					`<@${message.author.id}> adlı yetkili <@${sahip}> adlı kişinin <@${pythonic}> adlı botunu reddetti!`
				);
			client.channels.cache.get(log).send(logembed);
			client.channels.cache.get(log).send(`<@${sahip}>, <@${pythonic}>`);
			const onaylandı = new Discord.MessageEmbed()
				...
				.setDescription(
					`<@${message.author.id}> adlı kişi, <@${sahip}> adlı kişinin <@${pythonic}> adlı botunu reddetti!`
				);
			const embedd = new Discord.MessageEmbed()
				.setTitle("Botunuz reddedildi!")
				.setDescription(
					`<@${message.author.id}> adlı yetkili <@${pythonic}> adlı botunuzu reddetti!`
				);
			client.users.cache.get(sahip).send(embedd);
			return message.channel.send(onaylandı);
		}
		...
		let pythonic = args[1];
		if (!pythonic)
			return message.channel.send("Lütfen redddedilecek botun idini belirtin!");
		let sahip = db.fetch(`botbaşvuruid_${message.guild.id}.${pythonic}`);
		if (!sahip)
			return message.channel.send("Böyle bir bot başvurusu yapılmamış!");
		let sıra = db.fetch(`botlistsıra_${message.guild.id}`);
	...
	if (args[0] == "bilgi") {
		...
		if (onaydurumvar == onaydurumvar) {
			var durum = onaydurumvar;
		}
		if (onaydurumvar == undefined) {
			var durum = "onay bekliyor";
		}
		const embed = new Discord.MessageEmbed()
			.setTitle(`Bot Bilgi Sistemi`)
			.setDescription(
				`:bust_in_silhouette:   **SAHİP BİLGİLERİ**  :bust_in_silhouette:  \n \n Bot sahibi: <@${sahip}> \n Sahip idi: ${sahip} \n \n :robot:  **BOT BİLGİLERİ**  :robot: \n \n Bot idi: ${engin} \n Bot prefixi: ${prefix} \n Botun dbl durumu: ${dbldurum} \n  Onay durumu: ${durum}`
			);
