...
exports.run = async (client, message, args) => {
	const embed = new discord.MessageEmbed()
		...
		.setDescription(
			`:pushpin:  __s.botlist ayarlar__ = Botlist ayarlarını görürsünüz \n \n  :pushpin:  __s.botlist kanal = Botlist kanal ayarlarsınız__ \n \n :pushpin:  __s.botlist devepoler-rol__ = Botlist devepoler rol ayarlarsınız \n \n :pushpin:  __s.botlist yetkili-rol__ = Botlist yetkili rol ayarlarısınız \n \n :pushpin:  __s.botlist log__ = Botlist log ayarlarsınız \n \n :pushpin:  __s.botlist başvuru-log__ = Botlist başvuru log ayarlarsınız \n \n :pushpin:  __s.botlist dm-takip__ = Botlist dm takip sistemini ayarlarsınız`
		)
		.setTimestamp()
	...
	if (args[0] == "kanal") {
		...
		const abcaklımagelmedinapamisim = new discord.MessageEmbed()
			...
			.setDescription(
				`Az önce botlist kanalını <#${plasmic.id}> olarak ayarlandınız.`
			)
			.setTimestamp()
			.setFooter(`${message.author.username} tarafından kullanıldı`);
	...
	if (args[0] == "devepoler-rol") {
		...
		const pythonic = new discord.MessageEmbed()
			.setTitle(`Başarılı bir şekilde ayarladınız`)
			.setDescription(
				`Az önce botlist devepoler rolünü <@&${plasmic.id}> olarak ayarladınız.`
			)
			.setTimestamp()
			.setFooter(`${message.author.username} tarafından kullanıldı`);
	...
	if (args[0] == "yetkili-rol") {
		...
		const pythonic = new discord.MessageEmbed()
			.setTitle(`Başarılı bir şekilde ayarladınız`)
			.setDescription(
				`Az önce botlist yetkili rolünü <@&${plasmic.id}> olarak ayarladınız.`
			)
			.setTimestamp()
			.setFooter(`${message.author.username} tarafından kullanıldı`);
		return message.channel.send(pythonic);
	...
	if (args[0] == "dm-takip") {
		...
		if (args[1] == "aç") {
			...
			const embed = new discord.MessageEmbed()
				.setTitle("Başarılı bir şekilde ayarladınız")
				.setDescription(
					`Az önce botlist dm takip sistemini aktif hale getirdiniz.`
				)
				.setTimestamp()
				.setFooter(`${message.author.username} tarafından kullanıldı`);
	...
	if (args[0] == "log") {
		...
		const pavyonikab = new discord.MessageEmbed()
			.setTitle(`Başarılı bir şekilde ayarladınız`)
			.setDescription(
				`Az önce botlist kanalını <#${kanal.id}> olarak ayarladınız.`
			)
			.setTimestamp()
			.setFooter(`${message.author.username} tarafından kullanıldı`);
	...
	if (args[0] == "başvuru-log") {
		...
		const pavyonikab = new discord.MessageEmbed()
			.setTitle(`Başarılı bir şekilde ayarladınız`)
			.setDescription(
				`Az önce botlist başvuru log kanalını <#${kanal.id}> olarak ayarladınız.`
			)
			.setTimestamp()
			.setFooter(`${message.author.username} tarafından kullanıldı`);
		return message.channel.send(pavyonikab);
	...
	if (args[0] == "ayarlar") {
		...
		if (logvar == undefined) {
			var log = "ayarlanmamış";
		}
		let başvurulogvar = db.fetch(`botlistbaşvurulog_${message.guild.id}`);
		if (başvurulogvar == başvurulogvar) {
			var başvurulog = `<#` + başvurulogvar + `>`;
		}
		if (başvurulogvar == undefined) {
			var başvurulog = "ayarlanmamış";
		}
		let yetkilivar = db.fetch(`botlistyetkili_${message.guild.id}`);
		if (yetkilivar == yetkilivar) {
			var yetkili = `<@&` + yetkilivar + `>`;
		}
		if (yetkilivar == undefined) {
			var yetkili = "ayarlanmamış";
		}
		let devrolvar = db.fetch(`botlistdevrol_${message.guild.id}`);
		if (devrolvar == devrolvar) {
			var devrol = `<@&` + devrolvar + `>`;
		}
		if (devrolvar == undefined) {
			var devrol = "ayarlanmamış";
		}
		let dmtakipvar = db.fetch(`botlistdmtakip_${message.guild.id}`);
		if (dmtakipvar == dmtakipvar) {
			var dmtakip = "açık";
		}
		if (dmtakipvar == undefined) {
			var dmtakip = "kapalı";
		}
		let kanalvar = db.fetch(`botlistkanal_${message.guild.id}`);
		if (kanalvar == kanalvar) {
			var kanal = `<#` + kanalvar + `>`;
		}
		if (kanalvar == undefined) {
			var kanal = "ayarlanmamış";
		}
		const embed = new discord.MessageEmbed()
			.setTitle(
				`:gear:  ${message.guild.name} adlı sunucunun botlist ayarları  :gear:`
			)
			.setDescription(
				`:closed_book:  Botlist log: ${log} \n \n :blue_book:  Botlist başvuru log: ${başvurulog} \n \n :bookmark:  Botlist yetkilisi: ${yetkili} \n \n :green_book:  Devepoler rol: ${devrol}  \n \n :orange_book:  Dm takip: ${dmtakip} \n \n :notebook_with_decorative_cover:  Botlist kanalı: ${kanal}`
			);
		return message.channel.send(embed);
	}
	if (args[0] == "sıfırla") {
		if (!args[1])
			return message.channel.send(
				"Sıfırlanacak şeyi de yazınız: `dm-takip` `kanal` `log` `başvuru-log` `devepoler-rol` `yetkili-rol` `hepsi`"
			);
		if (args[1] == "dm-takip") {
		...
		if (args[1] == "hepsi") {
			let kontrol = db.fetch(`botlistyetkili_${message.guild.id}`);
			if (!kontrol)
				return message.channel.send("Sistemin tamamı ayarlı değil!");
